...
image1 = cv2.imread('./files/'+filename)     
grayImage1 = cv2.cvtColor(image1, cv2.COLOR_BGR2GRAY)

image2 = cv2.imread('./files/'+filename)     
grayImage2 = cv2.cvtColor(image2, cv2.COLOR_BGR2GRAY)

image3 = cv2.imread('./files/'+filename)     
grayImage3 = cv2.cvtColor(image3, cv2.COLOR_BGR2GRAY)

# cascade xml 파일 선택
body_cascade = cv2.CascadeClassifier(cv2.data.haarcascades +'./files/haarcascade_fullbody.xml')
